...
$lang = [
    ...
    "success_text" => "Расчет выполнен!
        ...
        \nСтраховая сумма %world_total1% %valut%.
        \nНесчастный случай %world_total2% %valut%.
        \nДоп. опции: %options%
    ...
    "phone_text" => "Отлично! Осталось всего 2 шага для оплаты страховки. Отправте, пожалуйста, свой мобильный номер телефона или введите в формате +380ххххххххх для того, что бы наш менеджер с Вами связался.",
    "thank_text" => "Спасибо за заказ!\n<a href='%link%'>Ссылка на оплату</a>",
    "thank_text2" => "Спасибо за заказ!",
    ...
    "phone_error_text" => 'Номер телефона нужно ввести в формате +380ххххххххх',
    ...
    "enter_adress_text" => "Укажите Ваш адрес латиненецей (например <b>KIEV</b>).",
...
function OrderTotal($user_id){
    ...
    if($days_count > 30){
        if($days_count > 30 && $days_count <= 60){
            $order_total = $order_total / 60;
        }elseif($days_count > 60 && $days_count <= 90){
            $order_total = $order_total / 90;
        }elseif($days_count > 90 && $days_count <= 180){
            $order_total = $order_total / 180;
        }elseif($days_count > 180 && $days_count <= 365){
            $order_total = $order_total / 365;
        }
        $order_total = $order_total * $days_count;
    }
